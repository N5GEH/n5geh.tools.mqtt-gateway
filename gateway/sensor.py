...
class Lorawan(Client):
  ...
  "uplink_message": {
    ...
    def run(self):
        """Run the sensor.
        """
        self.connect(self.config["connection_settings"]["server_ip"],
                     1883,
                     keepalive=60,
                     bind_address="",
                     bind_port=0,
        ...
        while True:
            self.publish(topic=self.topic, payload=json.dumps(self.payload), qos=0, retain=False, properties=None)
            print(f"Published message to topic {self.topic}")
            time.sleep(5)
        
    def _on_connect(self, client: Client, userdata: Any, flags: int, rc: int, properties: Properties = None):
