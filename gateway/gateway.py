...
class Gateway(IoTAMQTTClient):
    def __init__(self):
        ...
        self.device_topics = {}
        self.header = FiwareHeader(service='mqtt_gateway', service_path='/mqtt_gateway')
        self.iotac = IoTAClient(
            url=f"http://{config['connection_settings']['server_ip']}:{config['connection_settings']['iota_port']}",
            fiware_header=self.header)
        self.cb = ContextBrokerClient(
            url=f"http://{config['connection_settings']['server_ip']}:{config['connection_settings']['orion_port']}",
            fiware_header=self.header)
    ...
    def get_device_topic(self, device_id):
        if device_id not in self.device_topics:
    ...
    def remove_device_topic(self, device_id):
